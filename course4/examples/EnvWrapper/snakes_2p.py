...
class snakes_2p(BaseWrapper):
    ...
    def step(self, action, train=True):
        ...
        step_reward = reward  # 使用原Reward
        # step_reward = reward + torch.clamp(step_reward-reward, )
        # print(step_reward)
...
def get_reward(info, snake_index, reward):
    ...
    for i in snake_index:
        ...
        step_reward[i] += -min(dists)  # 奖励值=10-蛇头到豆子最小距离

        if i == 0:
            snake_dists = [np.sqrt(np.sum(np.square(other_snake - self_head))) for other_snake in snakes_position[1]]
            step_reward[i] += min(snake_dists)  # 惩罚值=蛇头到其他蛇蛇身最短距离-10
        else:
            snake_dists = [np.sqrt(np.sum(np.square(other_snake - self_head))) for other_snake in snakes_position[0]]
            step_reward[i] += min(snake_dists)
        '''if reward[i] > 0:
            # 如果现在状态不错 保持
            step_reward[i] += 20
        else:
            self_head = np.array(snake_heads[i])
            dists = [np.sqrt(np.sum(np.square(other_head - self_head))) for other_head in beans_position]
            step_reward[i] -= min(dists)  # 惩罚值=蛇头与豆子最小距离
            if reward[i] < 0:
                # 如果现在状态很差 惩罚
                step_reward[i] -= 10
        '''
        # print("reward:", step_reward[i]) 一般在[-1,4]
